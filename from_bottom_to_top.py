...
def calculate_pseudo_leaf_density_rate(image_with_border, pseudo_density_mask, crop_ratio_h: int, border_color):
    """
    ...
    Args:
        ...
        pseudo_density_mask (numpy.ndarray): The mask image representing pseudo leaf density.
    ...
    height, width, _ = pseudo_density_mask.shape
    ...
    cropped_mask = pseudo_density_mask[start_height:end_height]

    # Calculate the rate of pseudo leaf density (ratio of black pixels in the cropped area)
    ...
    mask_with_crop_border = cv2.rectangle(
        pseudo_density_mask, (0, start_height), (width, end_height), color=border_color, thickness=3
    )
...
@click.option(
    "--output-csv-name", "output_csv_name", default="pseu_rate_bottom.csv", type=click.Path(), help="出力csvの名前."
)
def main(input_path, conf_path, output_dir, output_csv_name):
    ...
    for i in range(len(config.horizontal_crop_ratio_list)):
        density_rate, border_draw_img, pseudo_leaf_density_mask_3ch = calculate_pseudo_leaf_density_rate(
            border_draw_img, pseudo_leaf_density_mask_3ch, config.horizontal_crop_ratio_list[i], config.border_color[i]
        )
        rate_list.append(density_rate)

        # RoI: Region of Interest
        print(f"RoI height / Image height: {config.horizontal_crop_ratio_list[i]}, leaf density rate: {density_rate}")
    ...
    # 結果画像を出力する
    img_h = cv2.hconcat([border_draw_img, pseudo_leaf_density_mask_3ch])
    output_image_path = str(output_dir_pathlib / f"{input_pathlib.stem}_rate_img.jpg")
    cv2.imwrite(output_image_path, img_h)
